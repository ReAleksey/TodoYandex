...
class LocalDataSource(private val todoItemDao: TodoItemDao) {
    ...
    suspend fun insertItem(item: TodoItem) = todoItemDao.insertItem(item)

    suspend fun updateItem(item: TodoItem) = todoItemDao.updateItem(item)

    suspend fun deleteItem(item: TodoItem) = todoItemDao.deleteItem(item)

    suspend fun deleteAllItems() = todoItemDao.deleteAllItems()

    suspend fun insertItems(items: List<TodoItem>) = todoItemDao.insertItems(items)
